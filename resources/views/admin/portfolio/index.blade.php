        ...
        <div class="col-12">
            <div class="card card-primary card-outline">
                <div class="card-header">
                    ...
                    <a class="btn btn-primary btn-sm btn-flat float-right" href="{{route('portfolio.add')}}"><i class="fas fa-plus-circle"></i> Create</a>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                        ...
                        <tbody>
                            ...
                            @foreach($about as $row)
                                <tr>
                                    <td>{{$i++;}}</td>
                                    <td>{{$row->name}}</td>
                                   <td>{{ $row->category == 1 ? 'Website' : ($row->category == 2 ? 'Software' : '') }}</td>
                                    <td>{{$row->link}}</td>
                                    <td> 
                                       
                                         <img src="{{ $row->image ? asset('uploads/portfolio/' . $row->image) : asset('uploads/avatar.png') }}" width="100" height="70" alt="Description Image">

                                    </td>
                                    <td>
                                        <a class="btn btn-info btn-sm btn-flat" href="{{route('portfolio.edit', $row->id)}}">
        ...
        $("#delete_btn").on('click', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    Toast.fire({
                        icon: 'success',
                        title: 'portfolio has been deleted successfully.'
                    });
                },
