...
<div class="card card-primary card-outline">
    ...
    <form id="EditUser" method="post" action="{{ route('portfolio.update') }}" enctype="multipart/form-data">
        @csrf
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        <label>Name</label>
                        <input type="text" name="name"  tabindex="1" class="form-control" value="{{ !empty($user['name']) ? $user['name'] : '' }}">
                 ...
                 <div class="col-md-4">
                    <div class="form-group">
                        <label>Category {{$user['category']}}</label>

                        <select name="category"  class="form-control">
                            <option value="1" {{ $user['category'] == 1 ? 'selected' : '' }}>Website</option>
                            <option value="2" {{ $user['category'] == 2 ? 'selected' : '' }}>Software</option>
                        </select>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        <label>Link</label>
                         <input type="text" name="link" tabindex="2" class="form-control" value="{{ !empty($user['link']) ? $user['link'] : '' }}">
					...
					<div class="form-group">
						...
						<div id="photo_preview" class="mt-2">
						    @if(!empty($user) && !empty($user) && !empty($user['image']))
						        <img src="{{asset('uploads/portfolio/'.$user['image'])}}" alt="Employee Photo" style="width: 150px; height: 150px;">
						    @else
